...
| Local authentication users | Same host database, `auth_users` table | Scoped identities are queryable centrally; credentials are salted scrypt hashes and are never returned by APIs. The JSON body carries each member's per-project permission sets (`UserPermissionSetSchema`). |
